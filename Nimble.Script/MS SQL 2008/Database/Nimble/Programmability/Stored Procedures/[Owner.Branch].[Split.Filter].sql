...
IF (EXISTS(
		...
		WHERE 
			...
			O.[type]	= 'P'				AND
			O.[name]	= 'Split.Filter'))
	DROP PROCEDURE [Owner.Branch].[Split.Filter];
GO

CREATE PROCEDURE [Owner.Branch].[Split.Filter]
(
	@predicate		XML,
...
--	Filter by entities
	...
	FROM [Common].[Criteria.Entity](@predicate.query('/*/BranchSplits')) X;
	IF (@criteriaValueExist = 1) BEGIN
		SET @criteriaValue = @criteriaValue.query('/*/BranchSplit');
		DECLARE @entities TABLE ([Id] UNIQUEIDENTIFIER);
		INSERT @entities SELECT * FROM 
		(
			SELECT DISTINCT E.[Id]
			FROM [Common].[Generic.Entities](@criteriaValue) X
			CROSS APPLY [Owner.Branch].[Split.Entity](X.[Entity]) E
		) E
		...
		IF (@@ROWCOUNT > 0)
			IF (@isFiltered = 0)
				...
				ELSE
					INSERT @split SELECT S.[SplitId] FROM [Owner.Branch].[Split] S
		...
		IF (@organisationIsFiltered = 1) BEGIN
			...
			ELSE
				IF (@isExcluded = 0)
					...
					LEFT JOIN
					(
						SELECT S.[SplitId] FROM [Owner.Branch].[Split] S
						INNER JOIN	@organisation	X	ON	S.[SplitOrganisationId]	= X.[Id]
					)	S	ON	X.[Id]	= S.[SplitId]
...
--	Filter by organisations
	...
	IF (@organisations IS NOT NULL) BEGIN
		...
		ELSE
			DELETE X FROM @split				X 
			INNER JOIN	[Owner.Branch].[Split]	S	ON	X.[Id]					= S.[SplitId]
...
--	Filter by codes
	...
	IF (@criteriaValueExist = 1) BEGIN
		...
		IF (@@ROWCOUNT > 0)
			IF (@isFiltered = 0)
				IF (@isExcluded = 0)
					INSERT @split SELECT DISTINCT S.[SplitId] FROM [Owner.Branch].[Split] S
			...
			ELSE
				IF (@isExcluded = 0)
					...
					LEFT JOIN
					(
						SELECT DISTINCT S.[SplitId] FROM [Owner.Branch].[Split] S
						INNER JOIN	@codes	X	ON	S.[SplitCode]	LIKE X.[Code]
					)	S	ON	X.[Id]	= S.[SplitId]
					WHERE S.[SplitId] IS NULL;
				ELSE
...
--	Filter by split branch types
	SELECT 
		...
		@criteriaValue		= X.[CriteriaValue],
		@criteriaValueExist	= X.[CriteriaValueExist]
	FROM [Common].[Criteria.Entity](@predicate.query('/*/SplitBranchTypes')) X;
	IF (@criteriaValueExist = 1) BEGIN
		DECLARE @splitBranchTypes TABLE ([SplitBranchType] NVARCHAR(MAX));
		INSERT @splitBranchTypes SELECT DISTINCT * FROM [Common].[Enum.Entities](@criteriaValue);
		IF (@@ROWCOUNT > 0)
			IF (@isFiltered = 0)
				IF (@isExcluded = 0)
					INSERT @split SELECT DISTINCT S.[SplitId] FROM [Owner.Branch].[Split] S
					INNER JOIN	@splitBranchTypes	X	ON	S.[SplitBranchType]	LIKE X.[SplitBranchType];
				ELSE 
					INSERT @split SELECT DISTINCT S.[SplitId] FROM [Owner.Branch].[Split] S
					LEFT JOIN	@splitBranchTypes	X	ON	S.[SplitBranchType]	LIKE X.[SplitBranchType]
					WHERE X.[SplitBranchType] IS NULL;
			ELSE
				IF (@isExcluded = 0)
					DELETE X FROM @split	X
					LEFT JOIN
...
--	Apply filters
	IF (@isFiltered = 0)
		SELECT @number = COUNT(*) FROM [Owner.Branch].[Split] S;
	ELSE
		IF (@isExcluded = 0)
